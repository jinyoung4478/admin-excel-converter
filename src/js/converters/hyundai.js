...
// 메인 변환 함수
function convertData(originWorkbook, mapping, fileName) {
...
// 결과 엑셀 생성
function createResultWorkbook(result) {
...
function initUI(container) {
    ...
    // 파일 입력 설정
    const originInput = FileInputManager.setup(
        'hyundai-originFile',
    ...
    const mappingInput = FileInputManager.setup(
        'hyundai-mappingFile',
    ...
    document.getElementById('hyundai-convertBtn').addEventListener('click', async () => {
        try {
            ...
            const originData = await ExcelCore.readFile(originFile);
            const mappingData = await ExcelCore.readFile(mappingFile);

            const mapping = parseMappingTable(mappingData);
            const result = convertData(originData, mapping, originFile.name);
            const workbook = createResultWorkbook(result);

            const outputFileName = originFile.name.replace(/\.xlsx?$/i, '_result.xlsx');
            ExcelCore.downloadExcel(workbook, outputFileName);

            StatusManager.success('hyundai-status', `변환 완료! ${result.data.length}건 추출`);
        } catch (error) {
            console.error(error);
